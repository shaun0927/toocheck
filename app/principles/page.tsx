...
import { HudLabel, NeutralBadge, RegistrationMarks } from '@/components/domain';
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-8 px-6 py-12">
        ...
        {PRINCIPLES.map((p, i) => (
          <li key={p.id} className="relative hud-panel">
            ...
            <details className="group" open={i === 0}>
              <summary className="flex cursor-pointer items-start justify-between gap-3 px-5 py-4 marker:hidden [&::-webkit-details-marker]:hidden">
                <div className="space-y-1">
                  <p className="mono mono-10 text-cyan">[{String(i + 1).padStart(2, '0')}] // {p.en}</p>
                  <h2 className="font-ko text-xl font-bold text-ink">{p.title}</h2>
                  <p className="text-sm text-ink/70">{p.summary}</p>
                </div>
                <span aria-hidden className="mono mono-10 text-dim transition-transform group-open:rotate-180">▾</span>
              </summary>
      ...
      <aside className="hud-panel p-5">
        <HudLabel tone="lime">CONTACT // OPS</HudLabel>
