  ...
  return (
    <main className="mx-auto max-w-5xl space-y-12 px-6 py-10">
      <header className="border-b border-hair pb-6">
        <HudLabel tone="lime">PREVIEW // DESIGN_SYSTEM</HudLabel>
      ...
      <Section title="StatusChip / HudLabel / LimeStamp">
        <div className="flex flex-wrap gap-3">
          <StatusChip tone="live"><span>DATA.OK</span><ChipDivider /><span>2026.05.20</span></StatusChip>
          <StatusChip tone="pending"><span>DATA.PENDING</span><ChipDivider /><span>자료 확인 중</span></StatusChip>
          <StatusChip tone="neutral"><span>SOURCE.SET</span><ChipDivider /><span>4 LINKS</span></StatusChip>
          <HudLabel tone="cyan">DISCLOSURE // 04</HudLabel>
          <HudLabel tone="lime">SIGNAL.SUBSCRIBE</HudLabel>
          <LimeStamp>검수 완료 // 2026.05.20</LimeStamp>
        </div>
      </Section>

      <Section title="NeutralBadge tones">
        <div className="flex flex-wrap gap-2">
          <NeutralBadge tone="neutral">NEUTRAL</NeutralBadge>
          <NeutralBadge tone="muted">MUTED</NeutralBadge>
          <NeutralBadge tone="info">INFO 정보</NeutralBadge>
          <NeutralBadge tone="check">CHECK 확인 권장</NeutralBadge>
          <NeutralBadge tone="attention">ATTENTION 주의 깊게 확인</NeutralBadge>
          <NeutralBadge tone="cyan">CYAN</NeutralBadge>
          <NeutralBadge tone="lime">LIME</NeutralBadge>
        </div>
      </Section>

      <Section title="CheckCard severity 3종">
      ...
      <Section title="DataPendingNote · SourceLink · AssetBreakdownBar">
        <DataPendingNote />
      ...
      {disc1 ? (<Section title="DisclosureCard — 1번"><DisclosureCard disclosure={disc1} /></Section>) : null}
      {disc3 ? (<Section title="DisclosureCard — 3번 (체납 + 부동산 상위)"><DisclosureCard disclosure={disc3} /></Section>) : null}

      <Section title="PromiseCard">
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="space-y-3">
      <h2 className="mono mono-10 text-dim">// {title.toUpperCase()}</h2>
      {children}
    </section>
  );
}
